...
- **じまんカード (QR共有)**: Bボタン長押しで成績をQR表示。スマホで読むと Pages のプロフィールカードが開き、Xでシェアできる
...
- **ミニゲーム（Bボタンのみ・会話主体）**: 5びょうあて／反射神経／連打／ラッキー7／がまんくらべ の5種＋IMU搭載機は「かたむけて エサとり」。結果は金・銀・銅メダルで評価
- **たまご**: ごきげん100&満腹80でたまごを産み、新しいペットが生まれる（友達・たまご・実績は引き継ぎ）
- **永続化**: ステータス・実績はNVSに保存され、電源を切っても消えない
...
| ボタンB（短押し） | ミニゲーム選択（メニュー内では短押しで送り・長押しで決定） |
| ボタンB（長押し） | じまんカード（QR）を表示 |
