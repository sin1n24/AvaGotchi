...
const char *FAVORITES[] = {
  "おさかな", "おにく", "くだもの", "おこめ", "おやつ",   // 普通の食べ物
  "ねじ", "でんち", "はんだ", "かみ", "ぼると",           // ロボットっぽい
  "アルゴリズム", "プラスチック",                          // ちょっと意外
};
...
const char *achievementTitle() {
  ...
  if (pet.st.battleWins >= 10)   return "ちえくらべおうじゃ";
...
void battleWithFriend(const FriendInfo &fi) {
  ...
  delay(2500);
  say("ちえくらべ！ " + String(myPow) + " 対 " + String(fi.power), 3000);
  delay(3000);
  if (myPow > fi.power) {
    pet.st.battleWins++;
...
void runDemo() {
  ...
  pet.feed();
  avatar.setExpression(Expression::Happy);
  say(String(FAVORITES[gFavorite]) + " もぐもぐ！", 2500);
  for (int i = 0; i < 4; i++) {
...
void setup() {
  ...
  gHue = id % 360;
  gFavorite = (id / 4) % FAVORITES_COUNT;
