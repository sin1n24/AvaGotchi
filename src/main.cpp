...
// ---- 逆さのとき「文字だけ」180度回転して表示（顔・吹き出しはそのまま） ----
void drawUpsideMessage(const char *msg) {
  auto &lcd = M5.Display;
  M5Canvas cv(&lcd);
  cv.setColorDepth(16);
  if (!cv.createSprite(240, 28)) return;  // メモリ確保できなければ何もしない
  cv.fillScreen(TFT_BLACK);
  cv.setFont(&fonts::efontJA_16);
  cv.setTextColor(TFT_WHITE);
  cv.setTextDatum(middle_center);
  cv.drawString(msg, 120, 14);
  // 黒を透過させて180度回転で画面へ（背景・顔は残る）
  cv.pushRotateZoom(lcd.width() / 2, 40, 180, 1.0f, 1.0f, (uint16_t)TFT_BLACK);
  cv.deleteSprite();
}
  ...
  if (haveAccel) {
    ...
    if (upsideNow) {
      if (upsideStartMs == 0) upsideStartMs = now;
        ...
        upsideNotified = true;
        isUpsideDown = true;
        avatar.suspend();                      // 顔・吹き出しは今の向きのまま静止
        drawUpsideMessage("さかさまだよ！");    // 文字だけ180度回転して表示
      }
    } else {
      upsideStartMs = 0;
      upsideNotified = false;
      if (isUpsideDown) {        // 通常表示へ復帰
        isUpsideDown = false;
        avatar.resume();
      }
    }
